...
const QUEUE = [
  { recordId: "ZAF-VEN-060", file: "south-africa-padel-verified.json", country: "South Africa" },
  { recordId: "ZAF-VEN-061", file: "south-africa-padel-verified.json", country: "South Africa" },
  { recordId: "ZAF-VEN-062", file: "south-africa-padel-verified.json", country: "South Africa" },
] as const;
...
async function main() {
  ...
  const prevPath = resolve(DATA, "tick-ingest-2026-09-26T18.json");
  let prev: { results?: Array<Record<string, unknown>> } = {};
  try {
  ...
  console.log(JSON.stringify({ outPath: prevPath, ok: results.filter((r) => r.ok).length, total: results.length }));
