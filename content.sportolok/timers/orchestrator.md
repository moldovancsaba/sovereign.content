...
**LLM:** Cloud Agent (YOU) are the LLM. NO OLLAMA. NO AI GATEWAY. NO EXTERNAL LLM.
You perform all cognitive tasks: scoring quality, writing About descriptions, extracting structured facts.
...
4) When ingest key present: npm run catalog:quality-loop (this folder). Cloud Agent acts as LLM.
...
9) NO OLLAMA. NO AI GATEWAY. You (Cloud Agent) are the LLM for all AI tasks.
```
